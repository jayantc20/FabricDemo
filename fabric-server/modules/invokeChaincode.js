...
const logger = helper.getLogger("invokeTransaction");
...
const invokeChaincode = async (
  ...
  try {
    ...
    for (const i in proposalResponses) {
      if (proposalResponses[i] instanceof Error) {
        ...
        error_message = util.format(
          "invoke chaincode proposal resulted in an error :: '%s'",
        ...
        all_good = false;
        error_message = util.format(
          "invoke chaincode proposal failed for an unknown reason '%s'",
          proposalResponses[i]
        );
        logger.error(error_message);
      }
    ...
    if (all_good) {
      ...
      event_hubs.forEach((eh) => {
        ...
        let invokeEventPromise = new Promise((resolve, reject) => {
          ...
          eh.registerTxEvent(
            ...
            (tx, code, block_num) => {
              logger.info(
                "The chaincode invoke chaincode transaction has been committed on peer "+ eh.getPeerAddr()
              );
              logger.info(
      ...
      logger.debug(util.format("Response : ${ results }"));
      ...
      for (let i in results) {
        ...
        logger.debug(
          "Event results for event hub : " +event_hub.getPeerAddr()
        );
